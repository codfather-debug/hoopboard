...
// date format: 'YYYYMMDD' or null for today
export async function getNBAScoreboard(date = null) {
  const dateParam = date ? `?dates=${date}` : ''
  const res = await fetch(`${ESPN_BASE}/nba/scoreboard${dateParam}`, { next: { revalidate: 60 } })
  if (!res.ok) return []
  const data = await res.json()
  return data.events || []
}

export async function getNCAA_MBScoreboard(date = null) {
  const dateParam = date ? `?dates=${date}` : ''
  const res = await fetch(`${ESPN_BASE}/mens-college-basketball/scoreboard${dateParam}`, { next: { revalidate: 60 } })
  if (!res.ok) return []
  const data = await res.json()
  return data.events || []
}
...
// Parse a game from a summary header (fallback when scoreboard doesn't have the game, e.g. past dates)
export function parseGameFromSummaryHeader(header, league) {
  ...
  return {
    id: header.id,
    league,
    name: header.shortName || `${away?.team?.abbreviation} @ ${home?.team?.abbreviation}`,
...
// Parse the first scoring play from a plays array
export function getFirstScorer(plays) {
  if (!plays || plays.length === 0) return null
  const play = plays.find(p => p.scoringPlay)
  if (!play) return null
